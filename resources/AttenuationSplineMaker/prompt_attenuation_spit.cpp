...
int main(int argc, char* argv[]){
  ...
  if(argc != 12){
      printf("ERROR:USAGE: program influx inearth inxsec outdir index dmsq41 [eVˆ2] th14 th24 th34 del14 del24 [rad] \n");
      exit(0);
  } else {
      input_flux_path  = argv[1];
      input_earth_path = argv[2];
      input_xsec_dir   = argv[3];
      output_path      = argv[4];
      index            = atoi(argv[5]);
      dm41sq           = atof(argv[6]);
      th14             = atof(argv[7]);
  ...
  for(std::string primary_type : {"neutrino","antineutrino"}){
    ...
    if(doStack){
        ...
        spline.write_fits(output_path+"/attenuation_spline_atmPrompt_" + argv[5] + "_" + primary_type + ".fits");
    }
  }
